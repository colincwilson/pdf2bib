...
pdf2doi.config.set('save_identifier_metadata', \
    config.get('save_identifier_metadata'))
...
def pdf2bib(target):
    ...
    if path.isdir(target):
        ...
        pdf_files = [
            f for f in listdir(target) if (f.lower()).endswith('.pdf')
        ]
        ...
        papers = [
        ]  #For each pdf file in the target folder we will store a dictionary inside this list
        for f in pdf_files:
            logger.info("................")
            file = target + f
...
def pdf2bib_singlefile(filename):
    ...
    logger = logging.getLogger("pdf2bib")
    logger.info(
        f"Trying to extract data to generate the BibTeX entry for the file: {filename}"
    )
    logger.info(f"Calling pdf2doi...")
    result = pdf2doi.pdf2doi(filename)
    if result['identifier'] == None:
        logger.error(
            "It was not possible to find a valid identifier for this file.")
        result['metadata'] = None
        result['bibtex'] = None
        return result
    if not (isinstance(result['validation_info'], str)
            or isinstance(result['validation_info'], dict)):
        result['metadata'] = None
        result['bibtex'] = None
        logger.error(
            "The validation_info returned by pdf2doi is not a string or valid dictionary. It is not possible to extract BibTeX data."
        )
        return result
    ...
    if result["identifier_type"] == "arxiv ID":
        ...
        metadata = bibtex_makers.parse_bib_from_dxdoiorg(
            result['validation_info'],
            method=pdf2doi.config.get('method_dxdoiorg'))
...
def save_bibtex_entries(filename_bibtex, results, clipboard=False):
    ...
    if isinstance(filename_bibtex, str):
        path_filename_bibtex = path.dirname(
            results[0]['path']) + config.get('separator') + filename_bibtex
        try:
            with open(path_filename_bibtex, "w",
                      encoding="utf-8") as text_file:
                text_file.write(text)
                print(
                    f'All available bibtex entries have been stored in the file {filename_bibtex}'
                )
        except Exception as e:
            print(e)
            print(
                f'A problem occurred when trying to write into the file {filename_bibtex}'
            )
    if clipboard:
        import pyperclip
        try:
            pyperclip.copy(text)
            print(
                f'All available bibtex entries have been stored in the system clipboard'
            )
        except Exception as e:
            print(e)
            print(
                f'A problem occurred when trying to write into the system clipboard'
            )
...
def main():
    ...
    parser.add_argument(
        ...
        "--verbose",
        help=
        "Increase verbosity. By default (i.e. when not using -v), only the text of the found bibtex entries will be printed as output.",
        action="store_true")
    parser.add_argument(
        "-nostore",
        "--no_store_identifier_metadata",
        help=
        "pdf2bib uses the library pdf2doi to find the DOI/identifier of a publication. By default, anytime an identifier is found, pdf2doi also adds it to the metadata of the pdf file (if not present yet). By using this additional option, the identifier is not stored in the file metadata.",
        action="store_true")
    parser.add_argument(
        "-s",
        "--make_bibtex_file",
        ...
        help=
        "Create a text file inside the target directory, with name given by FILENAME_BIBTEX, containing the bibtex entry of each pdf file in the target folder (if any is found).",
        action="store")
    parser.add_argument(
        "-clip",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "Add a shortcut to pdf2bib in the right-click context menu of Windows. This allows you to copy the bibtex\
    ...
    )  #store the desired verbose level in the global config of pdf2bib. This will also automatically update the pdf2bib and pdf2doi logger level.
    logger = logging.getLogger("pdf2bib")
    ...
    if target == "":
        print(
            "pdf2bib: error: the following arguments are required: path. Type \'pdf2bib --h\' for a list of commands."
        )
        return
    ## END

    config.set('save_identifier_metadata',
               not (args.no_store_identifier_metadata))
    pdf2doi.config.set('save_identifier_metadata',
                       config.get('save_identifier_metadata'))
    ...
    if (args.verbose == False):
        print(
            f"(All intermediate output will be suppressed. To see additional output, use the command -v)"
        )
    ...
    if not (
            args.filename_bibtex or args.save_bibtex_clipboard
    ):  #If the user wants to save the bibtex entries on file or on the clipboard, we dont show them in the command prompt
        for result in results:
            if result['identifier']:
                print(result['bibtex'])
    ...
    save_bibtex_entries(args.filename_bibtex, results,
                        args.save_bibtex_clipboard)
